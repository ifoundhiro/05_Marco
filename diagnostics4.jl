...
type the_problem
    ...
    mutation_chunk!(s_t_nontempered,ε,acpt_vec,inputs,myrange(ε))
		...
		function inputs4chunking()
			##### Declare dictionary to hold variable.
			dict=Dict();
			...
			##### Specify SharedArray if chunking on single node.
			if this.exectype=="chunk"
				dict["ε_shared"]=this.ε_shared;		
			##### Specify regular array if chunking on multiple nodes.
			elseif this.exectype=="chunk2"
				dict["ε"]=this.ε;
			end
		...
		####################################################################
		##### Define function to execute mutation in chunks.
		##### Non-SharedArray version.
		####################################################################		
		function mutation_chunk2(inputs,range)
			#@show (range)  # display so we can see what's happening
		...
		##### Non-SharedArray version.
		####################################################################
		function myrange2(index::Int64,n::Int64)
			##### Obtain array of ranges.
			splits=[round(Int,s) for s in linspace(0,n,nworkers()+1)];
			##### Get range for current worker.
		...
		##### Non-SharedArray version.
		####################################################################
		function mutation_shared2(inputs)
			##### Declare output array.
			out=Array{Tuple{Array{Float64},Array{Float64},Float64},1}(
			this.n_particles);
			##### Wait until enclosed tasks are complete.
			@sync begin
				##### Loop through workers.
				for (index,worker) in enumerate(workers())
					##### Do not wait for enclosed tasks to complete.
					@async begin
						##### Obtain range for current worker.
						range=myrange2(index,this.n_particles);
						##### Delegate chunk to current worker.
						out[range]=remotecall_fetch(
						mutation_chunk2,worker,inputs,range);
					end
				end
			end
			##### Return output.
		...
		this.execute_mutation=function()
			...
			println("*****   Parallel environment: ",this.parenv,"\n")			
			...
			for t = 1:this.T       
				...
				for i=1:10
					...
					#############################################################
					elseif this.exectype=="chunk2"	
						out=mutation_shared2(inputs4chunking());
					end
					#############################################################
