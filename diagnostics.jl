...
#####			- Test displaying summary of outputs.
#####		08/04/2017:
#####			- Set display of summary statistics for output variables.
##########################################################################
##### Define module name.
module diagnostics
...
function test1(
	...
	m,system,TTT,sqrtS2,s0,P0,s_lag_tempered,ε,yt,nonmissing, 
	N_MH,c,n_particles,deterministic,μ,cov_s,s_t_nontempered=setup();
	##### Display prompt.	
	println("\n****************************")
	...
	for t = 1:T       
		...
		for i=1:10
			...
			if exectype=="pmap"
				...
				##### Run pmap.
				out = pmap(i->mutation_problem(c,N_MH,deterministic,system,
				yt,s_lag_tempered[:,i],ε[:,i],cov_s,nonmissing,distCall),
				1:n_particles);
			...
			elseif exectype=="parfor"
				...
				out = @sync @parallel (hcat) for i=1:n_particles
					mutation_problem(c,N_MH,deterministic,system,yt,
					s_lag_tempered[:,i],ε[:,i],cov_s,nonmissing,distCall)
				end
			...
			elseif exectype=="serial"
				...
				print("(not parallel) ")
				##### Run sequentially.
				out = [mutation_problem(c,N_MH,deterministic,system,
				yt,s_lag_tempered[:,i],ε[:,i],cov_s,nonmissing,distCall) 
				for i=1:n_particles]
			end  
			##### Set prompt counter.
		...
		round(minimum(s_t_nontempered),2),
		" max: ",round(maximum(s_t_nontempered),2)," mean: ",
		round(mean(s_t_nontempered),2)," sum: ",round(sum(s_t_nontempered),2))
		println("***** ε min: ",round(minimum(ε),2),
		" max: ",round(maximum(ε),2)," mean: ",
		round(mean(ε),2)," sum: ",round(sum(ε),2),"\n")		
	end   
	##### Display prompt.	
	println("*******************************")
