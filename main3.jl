...
##### Modifications:
##### 		08/09/2017: 
#####			- Duplicated from main.jl.
#####			- Modify for diagnostics3.jl.
#####			- Move addprocs logic to command line.  SharedArray does not
#####				seems to get mapped on RAN using addprocs_sge().
...
elseif ((exectype=="parfor" || exectype=="pmap" 
	|| exectype=="pmapbatch" || exectype=="chunk") && nworkers()==1)
	error("command line mismatch between exec type and # of workers")
end
...
myprob=diagnostics3.the_problem();
